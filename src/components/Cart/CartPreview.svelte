...
{#if Object.values($cartItems).length}
    <aside class="mx-auto px-4 py-16" in:fade={{ duration: 300, delay: 200 }} out:fade={{ duration: 200 }}>
        <hr />
        {#each Object.values($cartItems) as cartItem}
            {#if cartItem.quantity > 0}
                ...
                >
                    ...
                    <div class=" text-start font-black">
                        <p class>{cartItem.name}</p>
                        <p class="text-xs font-normal text-yellow-500 lg:text-sm">
                            R${cartItem.price} un
                        </p>
                    </div>
